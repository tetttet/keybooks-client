...
const UserBooksGallery: React.FC<UserBooksGalleryProps> = ({ user_id }) => {
  ...
  useEffect(() => {
    // для каждой книги загружаем ответы
    books.forEach((book) => {
      fetchResponses(user_id, book.id);
    });
  }, [books, user_id, fetchResponses]);

  if (booksLoading || responsesLoading) return <p>Загрузка...</p>;
  if (booksError)
    return <p className="text-red-500">Ошибка книг: {booksError}</p>;
  if (responsesError)
    return <p className="text-red-500">Ошибка ответов: {responsesError}</p>;
